...
bool AssetManager::loadRgb565(const char* path, uint16_t*& destination) {
  ...
  const size_t bytesRead =
      file.read(reinterpret_cast<uint8_t*>(destination), expectedBytes);
  file.close();
...
bool AssetManager::drawRgb565(DisplayManager& display, const char* path) {
  if (!loadRgb565(path, imagePixels_)) return false;
...
uint16_t* AssetManager::loadRgb565Asset(const char* path) {
  uint16_t* pixels = nullptr;
  if (!loadRgb565(path, pixels)) return nullptr;
  return pixels;
}
